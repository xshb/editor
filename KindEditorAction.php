...
class KindEditorAction extends Action {
    ...
    public function init() {
        ...
        $attr_path = empty($this->attr_path) ? 'upload/' : 'upload/'.$this->attr_path.'/';

       // $this->php_path =  dirname(__FILE__) . '/';
        $this->php_path =  $_SERVER['DOCUMENT_ROOT'] . '/';
        $this->php_url =  '/';
//根目录路径，可以指定绝对路径，比如 /var/www/attached/
        $this->root_path = $this->php_path . $attr_path;
//根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
        $this->root_url = $this->php_url . $attr_path;
//图片扩展名
//            $ext_arr = ['gif', 'jpg', 'jpeg', 'png', 'bmp'],
//文件保存目录路径
        $this->save_path = $this->php_path . $attr_path;
//文件保存目录URL
        $this->save_url = $this->php_url . $attr_path;
//定义允许上传的文件扩展名
//            $ext_arr = array(
...
//有上传文件时
        if (empty($_FILES) === false) {
            ...
            $save_path = $this->save_path . $tmp_path;
            $save_url = $this->save_url . $tmp_path;
            ...
            //新文件名
            $new_file_name = date("dHi") . '_' . rand(100, 999) . '.' . $file_ext;
            //移动文件
